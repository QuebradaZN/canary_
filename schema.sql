...
CREATE TABLE
    IF NOT EXISTS `coins_transactions` (
        `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
SELECT
    0 AS date,
    "default" AS boostname,
    0 AS raceid;

-- Table structure `boosted_creature`

CREATE TABLE
    IF NOT EXISTS `boosted_creature` (
        `boostname` TEXT,
        `date` varchar(250) NOT NULL DEFAULT '',
        `raceid` varchar(250) NOT NULL DEFAULT '',
        `looktype` int(11) NOT NULL DEFAULT "136",
...
CREATE TABLE
    IF NOT EXISTS `daily_reward_history` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
...
FOR EACH ROW BEGIN INSERT
	...
	INSERT INTO
	    `guild_ranks` (`name`, `level`, `guild_id`)
	VALUES ('Vice-Leader', 2, NEW.`id`);
	INSERT INTO
	    `guild_ranks` (`name`, `level`, `guild_id`)
	VALUES ('Member', 1, NEW.`id`);
	END //
...
CREATE TABLE
    IF NOT EXISTS `player_hirelings` (
...
CREATE TABLE
    IF NOT EXISTS `player_items` (
        ...
        CONSTRAINT `player_items_pk` PRIMARY KEY (`player_id`, `pid`, `sid`)
    ) ENGINE = InnoDB DEFAULT CHARSET = utf8;
...
CREATE TABLE
    IF NOT EXISTS `player_wheeldata` (
        ...
        CONSTRAINT `player_wheeldata_pk` PRIMARY KEY (`player_id`)
    ) ENGINE = InnoDB DEFAULT CHARSET = utf8;

-- Table structure `player_kills`

CREATE TABLE
    IF NOT EXISTS `player_kills` (
        `player_id` int(11) NOT NULL,
        `time` bigint(20) UNSIGNED NOT NULL DEFAULT '0',
...
CREATE TABLE
    IF NOT EXISTS `player_bosstiary` (
        `player_id` int NOT NULL,
