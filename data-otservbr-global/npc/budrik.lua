...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "mission", "quest" }, message:lower()) then
		...
		elseif player:getStorageValue(Storage.KillingInTheNameOf.BudrikMinos) == 0 then
			if player:getStorageValue(Storage.Quest.U8_5.KillingInTheNameOf.MonsterKillCount.MinotaurCount) + player:getStorageValue(Storage.CustomMonsterKillCount.OramondMinotaurs) + player:getStorageValue(Storage.CustomMonsterKillCount.CultMinotaurs) >= 3000 then
				npcHandler:say({
				...
				npcHandler:say("Come back when you have slain {3000 minotaurs!}", npc, creature)
			end
