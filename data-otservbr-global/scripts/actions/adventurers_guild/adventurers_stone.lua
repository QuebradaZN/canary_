local setting = {
	...
	-- {fromPos = Position(33793, 32746, 4), toPos = Position(33801, 32757, 4), townId = TOWNS_LIST.MARAPUR}
}
...
function adventurersStone.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if not isInTemple then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'Try to move more to the center of a temple or depot to use the spiritual energy for a teleport.')
		return true
	end
