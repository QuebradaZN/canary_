...
TEST_CASE("Register Coin Transaction", "[IntegrationTest]") {
	...
	error_t result;
	result = account.RegisterCoinsTransaction(account::COIN_ADD, 50, "Test Register Add Coin 1");
	CHECK(result == account::ERROR_NO);

	result = account.RegisterCoinsTransaction(account::COIN_ADD, 100, "Test Register Add Coin 2");
	CHECK(result == account::ERROR_NO);

	result = account.RegisterCoinsTransaction(account::COIN_REMOVE, 250, "Test Register Remove Coin 3");
	CHECK(result == account::ERROR_NO);

	result = account.RegisterCoinsTransaction(account::COIN_REMOVE, 500, "Test Register Remove Coin 4");
	CHECK(result == account::ERROR_NO);

	result = account.RegisterCoinsTransaction(account::COIN_ADD, 1000, "Test Register Add Coin 5");
	CHECK(result == account::ERROR_NO);
}
