...
bool Save::savePlayerAsync(Player* player) {
	if (player != nullptr) {
		std::shared_lock<std::shared_mutex> addListLock(listPlayerMutex);
		...
		savePlayerThread = std::jthread([player]() {
			while (!playersToSaveList.empty() && player != nullptr) {
				...
				if (currentPlayer != nullptr) {
					...
					removePlayerFromSaveQueue(currentPlayer);
				}
			}
...
bool Save::checkIfPlayerInList(Player* player) {
	std::shared_lock<std::shared_mutex> lock(listPlayerMutex);
	return Save::playersToSaveList.contains(player);
}
