...
void Player::onCreatureAppear(Creature* creature, bool isLogin) {
	...
	if (isLogin && creature == this) {
		...
		if (getLastLogout() != 0) {
			...
			offlineTime = std::min<int32_t>(getTimeNow() - getLastLogout(), 86400 * 21);
		} else {
			offlineTime = 0;
		}
	...
	if (player && player->getSkull() == SKULL_NONE) {
		if (player == this) {
			for (const auto &kill : unjustifiedKills) {
				if (kill.unavenged && (getTimeNow() - kill.time) < g_configManager().getNumber(ORANGE_SKULL_DURATION) * 24 * 60 * 60) {
					return SKULL_ORANGE;
				}
			}
		}
...
bool Player::hasKilled(const Player* player) const {
	for (const auto &kill : unjustifiedKills) {
		if (kill.target == player->getGUID() && (getTimeNow() - kill.time) < g_configManager().getNumber(ORANGE_SKULL_DURATION) * 24 * 60 * 60 && kill.unavenged) {
			return true;
		}
	}
...
void Player::addUnjustifiedDead(const Player* attacked) {
	...
	unjustifiedKills.emplace_back(attacked->getGUID(), getTimeNow(), true);
	...
	for (const auto &kill : unjustifiedKills) {
		const auto diff = getTimeNow() - kill.time;
		if (diff <= 4 * 60 * 60) {
			dayKills += 1;
		}
		if (diff <= 7 * 24 * 60 * 60) {
